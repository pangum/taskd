...
import (
	...
	column2 "github.com/pangum/taskd/internal/internal/repository/internal/column"
...
func (t *Task) GetsRunnable(
	...
	required := builder.Lte{
		column2.Times.String(): times, // 还没有到最大重试次数
	}

	// 可被运行的条件一：运行时间已到且状态是被认可可被重新执行
	created := builder.Lte{
		column2.Next.String(): time.Now(), // 运行时间已到
	}.And(builder.Eq{
		column2.Status.String(): kernel.TaskStatus_TASK_STATUS_CREATED, // 刚创建的任务
	}.Or(builder.Eq{
		column2.Status.String(): kernel.TaskStatus_TASK_STATUS_FAILED, // 已经处于错误状态的任务
	}))
	// 可被运行的条件二：任务已完成执行，但需要重启执行（可被循环执行的任务，达到下一次执行的条件）
	recyclable := builder.Eq{ // 已经完成的任务，需要重新执行
		column2.Status.String(): kernel.TaskStatus_TASK_STATUS_SUCCESS, // 已完成
	}.And(builder.Eq{
		column2.Times.String(): 0, // 次数被重置
	})
	// 可被运行的条件三：因各种问题中断执行
	interrupted := builder.Lte{
		column2.Next.String(): time.Now().Add(-maximum), // 超过配置时间的最大运行时间段
	}.And(builder.Eq{
		column2.Status.String(): kernel.TaskStatus_TASK_STATUS_RUNNING, // 运行中
	}.Or(builder.Eq{
		column2.Status.String(): kernel.TaskStatus_TASK_STATUS_RETRYING, // 重试中
	}))
	...
	err = t.db.Where(cond).Limit(count).OrderBy(column2.Created.Asc()).Find(tasks)
